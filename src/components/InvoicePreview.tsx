...
export function InvoicePreview({ invoice }: InvoicePreviewProps) {
  ...
  return (
    ...
    >
      ...
      {/* Line Items Table */}
      <table className={`w-full mb-8 ${useCustomHeader && settings.showLines ? 'border-collapse' : ''}`}>
        <thead>
          <tr className={useCustomHeader && settings.showLines ? 'border-b border-gray-200' : 'border-b text-xs text-gray-500'}>
        ...
        <tbody>
          {invoice.lineItems.length > 0 ? (
            invoice.lineItems.map((item, index) => (
              <tr key={item.id} className={useCustomHeader && settings.showLines ? 'border-b border-gray-100' : 'border-b'}>
